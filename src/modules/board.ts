...
function checkDiagonal(board: Player[][], y: number, x: number, move: (y: number, x: number) => {nY: number, nX: number}): boolean {
	...
	while(y < board.length && x < board[0].length && y >= 0 && x >= 0){
		...
		if(nY < 0 || nX < 0 || nX >= board[0].length || nY >= board.length) return false;
		
		count = board[y][x] === board[nY][nX] && board[y][x] !== null ? count + 1 : 0;
...
export function checkGameOver(board: Player[][]): boolean{
	
	{
		...
		for(let y = 0; y < board.length; y++){
			for(let x = 1; x < board[0].length; x++){
				countX = board[y][x] === board[y][x - 1] && board[y][x] !== null ? countX + 1 : 0;
	...
	
	{
		...
		for(let x = 0; x < board[0].length; x++){
			for(let y = 1; y < board.length; y++){
				countY = board[y][x] === board[y - 1][x] && board[y][x] !== null ? countY + 1 : 0;
...

	{
		...
		for(let x = 0; x < board[0].length; x++){
			// DownRight
			const diagDR = checkDiagonal(board, 0, x, (y, x) => {return {nY: y+1, nX: x+1}})
			if(diagDR) return true

			// DownLeft
			const diagDL = checkDiagonal(board, 0, x, (y, x) => {return {nY: y+1, nX: x-1}})
			if(diagDL) return true
		}

		for(let x = 0; x < board[0].length; x++){
			// UpRight
			const diagUR = checkDiagonal(board, board.length - 1, x, (y, x) => {return {nY: y-1, nX: x+1}})
			if(diagUR) return true

			// UpLeft
			const diagUL = checkDiagonal(board, board.length - 1, x, (y, x) => {return {nY: y-1, nX: x-1}})
